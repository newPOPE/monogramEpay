...
class EPlatbaPaymentRequest extends EPaymentDesSignedMessage implements IEPaymentHttpRedirectPaymentRequest, IEPaymentHttpPostPaymentRequest {
    ...
    public function validateData() {
        ...
        if (!preg_match("/^https?://.+$/i", $this->RURL)) return false;
    ...
    protected function getSignatureBase() {
        //echo "Signature base: ". $this->MID.$this->AMT.$this->VS.$this->CS.$this->RURL."\n";
        return $this->MID.$this->AMT.$this->VS.$this->CS.$this->RURL;
    }
    ...
    public function GetRedirectUrl() {
        $base = $this->urlBase."?MID={$this->MID}&AMT={$this->AMT}&VS={$this->VS}&CS={$this->CS}&RURL={$this->RURL}&SIGN={$this->SIGN}";
        if (!empty($this->SS))		$base .= "&SS={$this->SS}";
        if (!empty($this->DESC))	$base .= "&DESC=".urlencode($this->DESC);
        if (!empty($this->REM))		$base .= "&REM=".urlencode($this->REM);
        if (!empty($this->RSMS))	$base .= "&RSMS=".urlencode($this->RSMS);
        return $base;
    }
