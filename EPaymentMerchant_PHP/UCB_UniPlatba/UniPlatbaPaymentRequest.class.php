...
class UniPlatbaPaymentRequest extends EPaymentDesSignedMessage implements IEPaymentHttpRedirectPaymentRequest {
    ...
    protected function validateData() {
        try {
            ...
            if (!preg_match('/^[0-9]{1,13}\\.[0-9]{2}$/i', $this->AMT)) throw new Exception('Amount must be a decimal number with 2 digits after period delimiter.');
            if (!preg_match('/^[0-9]{1,10}$/i', $this->MID)) throw new Exception('Merchant ID is in wrong format');
